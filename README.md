...
>
> Reassigning a `mut` binding now destroys the value it displaces, so a binding rotated through a loop holds steady instead of growing. One structural hole is left: the drop pass runs over bindings and nothing else, so **a value held inside another value is not released when the holder is** — a struct field, an array or tuple element, an enum payload, a newtype's inner value. It reaches every owning type: a `Vec`, a `String` or a `Tensor` in a field is as affected as a heap `string`.
>
