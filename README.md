...
| **Ownership & borrows** | Move-by-default, `Copy`, deterministic `Drop`, `&T` / `&mut T` with flow-sensitive exclusivity, lifetime elision and annotations; `pool { }` / `pool label { }` arena blocks, where what the block allocates comes from one bump region and goes back in a single store at its closing brace, and a type with a destructor needs `PoolAware` to live in one |
