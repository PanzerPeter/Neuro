...
| **Ownership & borrows** | Move-by-default, `Copy`, deterministic `Drop`, `&T` / `&mut T` with flow-sensitive exclusivity, lifetime elision and annotations; `pool { }` / `pool label { }` arena blocks, where what the block allocates comes from one bump region and goes back in a single store at its closing brace, and a type with a destructor implements `PoolAware` to live in one and is released by the arena's reverse-order sweep |
...
| 2D | Pool allocator: `pool` blocks, `PoolAware`, LIFO release at scope exit | In progress |
