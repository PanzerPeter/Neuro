...
| **`Option` / `Result`** | `Option<T>` and `Result<T, E>` from an implicit prelude — ordinary generic enums, available without a declaration; `??` unwraps either with a lazy fallback; `checked_add` / `checked_sub` / `checked_mul` report integer overflow as `Option::None` |
...
| 1G | Error handling, modules & prelude — `Option`/`Result` ✅, collections ✅, `checked_*` ✅, `??` ✅, `?`, multi-file modules, imports, prelude | 🔄 In progress |
