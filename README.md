...
| **Automatic differentiation** | `@grad` compiles a reverse-mode derivative of a tensor loss beside it, through branches and loops, at compile time and with no gradient tape; every derivative is checked against finite differences ([reference](docs/language-reference/autodiff.md)) |
...
over tensor arithmetic, `if` and `while`, and every derivative the compiler generates is measured against
