...
| **Modules & visibility** | Multi-file programs: every `.nr` file is a module and `mod.nr` directories nest; `import math::{sqrt}`, `import ./utils`, `as` renames and module aliases, and variant imports that make `Some(n)` / `None` read unqualified; declarations and struct fields are private until `export` opts them in |
...
| 1G | Error handling, modules & prelude — `Option`/`Result` ✅, collections ✅, `checked_*` ✅, `??` ✅, `val-else` ✅, `?` ✅, error-path outlining ✅, multi-file modules ✅, imports ✅, `export` visibility ✅, prelude | 🔄 In progress |
