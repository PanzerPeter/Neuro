...
Every runnable program in [examples/showcase/](examples/showcase/) combines several features at once and is pinned twice: to an expected exit code in [examples/expected.txt](examples/expected.txt), and to the exact text it prints in a sibling `.out` file. By-value tensor arithmetic, `@grad`, and GPU kernels are not shown here because they do not exist yet; tensor *construction* does, in [`showcase/model_shapes.nr`](examples/showcase/model_shapes.nr), and the in-place update in [`showcase/optimizer_step.nr`](examples/showcase/optimizer_step.nr). See the [Quick Roadmap](#quick-roadmap).
...
| 2B | Tensor core: `Tensor<T, [...]>`, literal coercion, move semantics, DLPack, slicing, shape generics, named dims, shape manipulation, dynamic shapes, reductions | In progress |
