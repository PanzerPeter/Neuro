...
| **Modules** | Multi-file programs: every `.nr` file is a module and `mod.nr` directories nest; `import math::{sqrt}`, `import ./utils`, `as` renames and module aliases, and variant imports that make `Some(n)` / `None` read unqualified |
