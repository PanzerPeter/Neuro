...
## Recent Updates
- 2026-07-19: Added `TokenKind::Dyn` keyword token for `dyn Trait` trait objects (§3.17). Reserves the word so it cannot be an identifier. Sits directly after `Trait` in declaration order. `impl` needed no new token — the existing `TokenKind::Impl` serves both `impl` blocks and the `impl Trait` bound.
