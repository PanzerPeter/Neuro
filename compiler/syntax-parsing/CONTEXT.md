...
- 2026-08-24: Statement-boundary fix. `parse_expr_inner` now treats a newline followed by `(`
  or `[` as a statement boundary, alongside the `*` case already there. It skipped newlines
  before consulting the next token's precedence, so the FOLLOWING line decided whether the
  expression continued — the inverse of the documented rule, where the line that ENDED decides
  (it ends with an operator, a comma, or an opening delimiter, all of which arrive here with no
  pending newline). `val a = f()` followed by a line `(2 + 3)` parsed as `f()(2 + 3)`, and a
  following `[1, 2]` as an index; with a callable on the left the misparse type-checked. A
  leading `.` still continues, since it cannot start a statement.
