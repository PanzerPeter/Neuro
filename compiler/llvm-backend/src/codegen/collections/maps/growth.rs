...
impl<'ctx> CodegenContext<'ctx> {
    ...
    pub(super) fn emit_hashed_grow_if_needed(
        ...
        let scaled_used = self.builder.build_int_mul(
            next_used,
            i64_ty.const_int(LOAD_NUMERATOR, false),
            "used.scaled",
        )?;
        let scaled_cap = self.builder.build_int_mul(
            capacity,
    ...
    pub(super) fn emit_hashed_rehash(
        ...
        let new_cap = self
            ...
            .into_int_value();
        let bytes = self.builder.build_int_mul(new_cap, stride, "cap.bytes")?;
        ...
        self.builder.build_store(cursor, zero)?;
        ...
        let live = self.builder.build_int_compare(
            IntPredicate::EQ,
            state,
        ...
        self.builder.build_call(free, &[old_buffer.into()], "")?;
        Ok(())
    }
    ...
    pub(super) fn probe_for_empty(
        ...
        let taken = self.builder.build_int_compare(
            IntPredicate::EQ,
            state,
            self.context.i8_type().const_int(STATE_FULL, false),
            "probe.taken",
