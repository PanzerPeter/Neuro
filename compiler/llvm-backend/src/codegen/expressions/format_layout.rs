...
impl<'ctx> CodegenContext<'ctx> {
    ...
    pub(crate) fn get_or_define_pad(&self) -> CodegenResult<FunctionValue<'ctx>> {
        ...
        let slack = self.builder.build_int_sub(width, len, "pad.slack")?;
        let half =
            self.builder
                .build_int_unsigned_div(slack, i64_type.const_int(2, false), "pad.half")?;
        let is_left = self.builder.build_int_compare(
            IntPredicate::EQ,
            align,
            i32_type.const_int(ALIGN_LEFT, false),
            "pad.left",
        )?;
        let is_center = self.builder.build_int_compare(
            IntPredicate::EQ,
            align,
        ...
        let zero_fill = self.builder.build_int_compare(
            IntPredicate::EQ,
            fill,
            i8_type.const_int(u64::from(b'0'), false),
            "pad.zero",
        )?;
        let non_empty = self.builder.build_int_compare(
            IntPredicate::UGT,
            len,
            i64_type.const_zero(),
            "pad.nonempty",
        ...
        let is_plus = self.builder.build_int_compare(
            IntPredicate::EQ,
            first,
            i8_type.const_int(u64::from(b'+'), false),
            "pad.is.plus",
        )?;
        let is_minus = self.builder.build_int_compare(
            IntPredicate::EQ,
            first,
            i8_type.const_int(u64::from(b'-'), false),
            "pad.is.minus",
        )?;
        let is_sign = self.builder.build_or(is_plus, is_minus, "pad.is.sign")?;
        self.builder
        ...
        self.builder.build_return(Some(&padded))?;
    ...
    pub(crate) fn get_or_define_quote(&self) -> CodegenResult<FunctionValue<'ctx>> {
        ...
        self.builder.build_return(Some(&quoted))?;
        self.end_helper(saved);
        Ok(func)
    }
    ...
    pub(crate) fn get_or_define_utf8(&self) -> CodegenResult<FunctionValue<'ctx>> {
        ...
        let wide = self.builder.build_int_z_extend(code, i64_type, "utf8.cp")?;
        let buf = self.build_malloc(i64_type.const_int(4, false), "utf8.buf")?;
        let count = self.builder.build_alloca(i64_type, "utf8.count")?;
        ...
        let below_ascii = self.builder.build_int_compare(
            IntPredicate::ULT,
            wide,
        ...
        let below_two = self.builder.build_int_compare(
            IntPredicate::ULT,
            wide,
        ...
        let below_three = self.builder.build_int_compare(
            IntPredicate::ULT,
            wide,
        ...
        self.builder.build_return(Some(&encoded))?;
