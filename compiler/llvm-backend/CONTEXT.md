...
diagnostics. They do not affect codegen elsewhere.
...
through to the caller's storage. It also seeds `type_env["self"]` so a `self.field = …`
...
- `string.slice(a..b)` / `.slice(a..=b)` → `BuiltinMethod::StringSlice`, lowered in
...
recorded in `LoopTargets.drop_scope_depth`. A panic aborts without running drops (no landing
...
- 2026-07-03: Generic functions (backend support). No generics reach the backend — hir-lowering
...
- 2026-07-02: Newtype declarations. A newtype is transparent at runtime: `Type::from_hir` erases
...
- 2026-07-02: Pattern matching (`codegen/expressions/matches.rs`). `codegen_match` evaluates the
  ...
  payload slot decoded by `decode_enum_payload_field` — the inverse of the payload pack), evaluates the
...
- 2026-06-30: Enums with associated data. New `Type::Enum(String)`; `compile` builds an
...
- 2026-06-29: Struct + array destructuring. `codegen/expressions/arrays.rs` gained
...
- 2026-06-28: Tuples. New backend `Type::Tuple(Vec<Type>)` (`from_hir`; `map_type` → anonymous
...
- 2026-06-19: Arrays. New `Type::Array { element, size }` (`from_ast`/`resolve_syntax_type`,
...
- 2026-06-18: `Drop` deterministic destruction. New `codegen/drops.rs` + `DropEntry`,
...
- 2026-06-16: `f16`/`bf16` half-precision primitives. New backend `Type::F16`/`Type::BF16` lower to
...
- 2026-06-15: `char` primitive type. New backend `Type::Char` lowers to LLVM `i32`
  ...
  points are < 2^21.
- 2026-06-15: `loop` as a value expression. `LoopTargets` gained `break_slot:
...
- 2026-06-15: Loop labels. `LoopTargets` gained `label: Option<String>`; `codegen_while` /
...
- 2026-06-09: `loop { ... }` statement. `codegen_loop` mirrors `codegen_while` without a
...
- 2026-06-09: Mutable borrows `&mut T` + deref `*`. `&mut place` lowers like `&place` —
  ...
  `Stmt::Expr` call path.
- 2026-06-08: Immutable borrows `&T`. New backend `Type::Reference(Box<Type>)` (+ `from_ast`,
...
- 2026-06-07: `struct.clone()` — `BuiltinMethod::StructClone` (`context.rs`); type-pass struct
...
- 2026-06-05: Struct functional-update. `codegen_struct_literal` (`codegen/structs.rs`) takes
...
- 2026-06-05: `string.clone()` — `BuiltinMethod::StringClone` + `(Type::String,"clone")` arm
...
- 2026-06-04: Panic runtime — new `panic.rs`; `compile` gained `source`/`source_path`;
...
- 2026-06-04: Fixed binary-operand type-map collision. A binary node and its leftmost
...
- 2026-06-02: `&&`/`||` short-circuit via `codegen_short_circuit` (phi over `logic.rhs`/
...
- 2026-06-02: Formalized string literal/runtime distinction (`STRING_NULL_TERMINATOR` named).
  Behaviour unchanged. See "String ABI".
- 2026-05-31: Integer primitive methods — `Wrapping`/`Saturating`/`Shr` in `BuiltinMethod`
  + `resolve_builtin_method`; lowered in `codegen_int_intrinsic`. See "Builtin Method ABI".
- 2026-05-31: Builtin method dispatch on primitive/string — `BuiltinMethod` enum +
  `resolve_builtin_method` + `builtin_methods` map (`context.rs`); first intrinsic `string.len()`.
- 2026-05-30: Integer overflow — `overflow_checks` (from `-O0`) gates `codegen_int_arith`
  (`with.overflow` + `llvm.trap` for debug, wrapping for release). See "Integer Overflow ABI".
- 2026-05-25: Float literal suffixes — `codegen_literal` dispatches `F32`/`F64`;
