...
| `types/`         | Primitive types, `char` literals, `f16`/`bf16` half-precision, literal suffixes, separators, casts, overflow, strings, string slices (`&string`), move semantics, immutable borrows (`&T`), borrow exclusivity (`&`/`&mut` aliasing rules), `@derive(Copy, Clone)`, type aliases |
