...
| [Automatic Differentiation](language-reference/autodiff.md) | `@grad` on functions and methods, `wrt:` selection of parameters and receiver fields, the derivative it generates, `.backward()` / `.grad()` / `.zero_grad()` and the borrows they end, its signature rules, which constructs a differentiated body may use, and how the derivative follows branches, loops, calls, function values and elementwise math |
