...
v2.1.0, `.is_nan()` in v2.2.0, buffered standard output in v2.3.0, and the codepoint-indexed
`.char_slice(range)` in v2.4.0; `.enumerate()` on arrays and ranges is next. Start at the top
of 2A in the roadmap, and move to `2B — Tensor Core` (`Tensor<T, [...]>` static tensor type
syntax) only once 2A is clear.
